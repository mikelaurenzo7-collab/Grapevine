#!/usr/bin/env python3
"""
AstraX AI - Fully Real Autonomous X Agent Platform (v1.1 - Thread Support + Production Polish)

Key upgrades in this round:
- Full multi-tweet thread posting (agent now creates 4-8 tweet threads)
- Real X analytics fetching (impressions, engagement on recent posts)
- Enhanced agent prompt for thread strategy
- Production logging + health checks
- Dynamic REDIRECT_URI + Railway-ready
...
async def call_llm(prompt: str) -> str:
    if not LLM_API_KEY:
        return json.dumps({"action": "POST_THREAD", "content": ["High-value update from AstraX AI - add your LLM_API_KEY for full power"], "reason": "Fallback mode"})
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            r = await client.post(f"{LLM_BASE_URL}/chat/completions", headers={"Authorization": f"Bearer {LLM_API_KEY}"}, json={"model": "gpt-4o-mini", "messages": [{"role": "system", "content": "You are Astra, the world's most effective autonomous X growth agent. You specialize in high-signal, value-first threads that drive engagement and growth. Always output ONLY valid JSON with: action (POST_THREAD or ANALYZE), content (array of 4-8 tweet strings for threads, or single string), reason (string). Never spam. Make every tweet add unique value."}, {"role": "user", "content": prompt}]})
            return r.json()["choices"][0]["message"]["content"]
    except Exception as e:
        logger.error(f"LLM error: {e}")
        return json.dumps({"action": "POST_THREAD", "content": ["Strategic update from AstraX"], "reason": str(e)})

async def post_thread(account_id: int, tweets: List[str]) -> Dict:
    """Post a real multi-tweet thread to X"""
    conn = get_db()
    ...
    async with httpx.AsyncClient() as client:
        for i, text in enumerate(tweets):
            ...
            reply_to = tweet_data["id"]  # Chain the next tweet
    
    return {"thread_ids": tweet_ids, "count": len(tweets)}

async def fetch_recent_analytics(account_id: int) -> Dict:
    """Fetch real performance data from X for the last 5 posts"""
    conn = get_db()
    row = conn.execute("SELECT access_token FROM accounts WHERE id=?", (account_id,)).fetchone()
    conn.close()
    if not row:
    ...
    async with httpx.AsyncClient() as client:
        ...
        tweets_resp = await client.get(f"https://api.x.com/2/users/{user_id}/tweets", 
            headers={"Authorization": f"Bearer {token}"}, 
            params={"max_results": 5, "tweet.fields": "public_metrics"})
...
async def run_agent_cycle():
    ...
    # Enhanced prompt for thread strategy
    prompt = f"""Current time: {datetime.now().isoformat()}
Goals: {config[0]}
Brand voice: {config[1]}
Niche: {config[2]}

Create ONE high-value 5-8 tweet thread that advances the goals. Each tweet must be under 280 characters, add unique insight, and drive engagement. Output ONLY JSON: {{"action": "POST_THREAD", "content": ["tweet 1", "tweet 2", ...], "reason": "why this thread"}}"""
    ...
    if decision.get("action") == "POST_THREAD" and isinstance(decision.get("content"), list):
        try:
            thread_result = await post_thread(aid, decision["content"])
            result["posted"] = thread_result
            conn.execute("INSERT INTO logs (timestamp, action, details, account_id) VALUES (?, ?, ?, ?)", 
                      (datetime.now().isoformat(), "THREAD", json.dumps(decision), aid))
            
            # Update analytics
            analytics = await fetch_recent_analytics(aid)
            conn.execute("INSERT OR REPLACE INTO analytics (account_id, date, impressions, engagements, posts) VALUES (?, ?, ?, ?, ?)",
                      (aid, datetime.now().date().isoformat(), analytics.get("impressions", 0), analytics.get("engagements", 0), analytics.get("posts", 0)))
        except Exception as e:
            result["error"] = str(e)
...
app = FastAPI(title="AstraX AI v1.1", lifespan=lifespan)

@app.get("/health")
async def health():
    return {"status": "healthy", "version": "1.1-thread-support", "timestamp": datetime.now().isoformat()}

@app.get("/auth/x")
async def start_oauth():
    if not X_CLIENT_ID or not X_CLIENT_SECRET:
        raise HTTPException(500, "X_CLIENT_ID and X_CLIENT_SECRET required")
...
async def status():
    ...
    analytics = conn.execute("SELECT impressions, engagements FROM analytics ORDER BY id DESC LIMIT 1").fetchone() or (0, 0)
    conn.close()
    return {
        "accounts": accs,
        "config": {"goals": cfg[0], "voice": cfg[1], "niche": cfg[2], "autonomous": bool(cfg[3])},
        "logs": [{"ts": l[0], "action": l[1], "details": l[2]} for l in logs],
        "analytics": {"impressions": analytics[0], "engagements": analytics[1]}
    }
